...
const server = http.createServer((req, res) => {
    ...
    if (page == "/") {
        ...
        let throws = `You choose ${
            userThrow[0].toUpperCase() + userThrow.slice(1)
        }. CPU chooses ${cpuThrow[0].toUpperCase() + cpuThrow.slice(1)}.`
        ...
        const winOrLose = () => {
            if (userThrow === cpuThrow) {
                ...
                (userThrow === `pikachu` && cpuThrow === `squirtle`) ||
                (userThrow === `squirtle` && cpuThrow === `geodude`) ||
                (userThrow === `geodude` && cpuThrow === `pikachu`)
            ) {
